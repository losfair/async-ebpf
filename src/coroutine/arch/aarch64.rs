...
use crate::coroutine::stack::{Stack, StackPointer};
use crate::coroutine::unwind::{
...
global_asm!(
  ...
  "mov x2, sp",
  // OpenBSD's arm64 BTI requires an indirect call (rather than a jump) when
  // entering the Rust coroutine function.
  "ldr x3, [x1, #8]",
  "blr x3",
...
pub unsafe fn switch_and_throw(
  forced_unwind: crate::coroutine::unwind::ForcedUnwind,
  sp: StackPointer,
  stack_base: StackPointer,
) -> (EncodedValue, Option<StackPointer>) {
  extern "C-unwind" fn throw(forced_unwind: crate::coroutine::unwind::ForcedUnwind) -> ! {
    extern crate std;
    use std::boxed::Box;
...
///
/// ```ignore
/// # use crate::coroutine::trap::TrapHandlerRegs;
...
pub unsafe fn setup_trap_trampoline<T>(
  ...
  TrapHandlerRegs {
    ...
    lr: stack_init_trampoline_return as *const () as u64,
  }
}
