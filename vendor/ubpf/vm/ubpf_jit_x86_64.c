...
#else
#define RCX_ALT R9
static int platform_nonvolatile_registers[] = {RBP, RBX, R12, R13, R14, R15};
...
translate(struct ubpf_vm* vm, struct jit_state* state, char** errmsg)
{
    ...
    /*
     * Set BPF R10 (the way to access the frame in eBPF) to shadow stack if enabled or RSP otherwise.
     */
    if (vm->jit_shadow_stack) {
        if(map_register(BPF_REG_10) != platform_parameter_registers[1]) {
            emit_mov(state, platform_parameter_registers[1], map_register(BPF_REG_10));
        }
    } else {
    ...
    /*
     * Load pointer offset into R12
     */
    emit_load_imm(state, R12, (int64_t) vm->jit_pointer_offset);
    ...
    for (i = 0; i < vm->num_insts; i++) {
        ...
        case EBPF_OP_STW:
            emit_load_imm(state, RCX, inst.imm);
            emit_masked_store(vm, state, S32, RCX, dst, inst.offset);
            break;
        case EBPF_OP_STH:
            emit_load_imm(state, RCX, inst.imm);
            emit_masked_store(vm, state, S16, RCX, dst, inst.offset);
            break;
        case EBPF_OP_STB:
            emit_load_imm(state, RCX, inst.imm);
            emit_masked_store(vm, state, S8, RCX, dst, inst.offset);
            break;
        case EBPF_OP_STDW:
            emit_load_imm(state, RCX, inst.imm);
            emit_masked_store(vm, state, S64, RCX, dst, inst.offset);
            break;
