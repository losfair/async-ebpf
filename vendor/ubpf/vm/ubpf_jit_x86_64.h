...
static inline void
emit_masked_load(const struct ubpf_vm *vm, struct jit_state* state, enum operand_size size, int src, int dst, int32_t offset) {
    assert(offset >= -32768 && offset <= 32767);

    if(vm->jit_pointer_mask) {
        if(src != RCX)
            emit_mov(state, src, RCX);
        emit_alu64_imm32(state, 0x81, 4, RCX, vm->jit_pointer_mask); // AND64
        emit_alu64(state, 0x01, R12, RCX); // ADD64
        emit_load(state, size, RCX, dst, offset);
    } else {
...
static inline void
emit_masked_store(const struct ubpf_vm *vm, struct jit_state* state, enum operand_size size, int src, int dst, int32_t offset) {
    assert(offset >= -32768 && offset <= 32767);

    if(vm->jit_pointer_mask) {
        if(dst != RCX)
            emit_mov(state, dst, RCX);
        emit_alu64_imm32(state, 0x81, 4, RCX, vm->jit_pointer_mask); // AND64
        emit_alu64(state, 0x01, R12, RCX); // ADD64
        emit_store(state, size, src, RCX, offset);
    } else {
