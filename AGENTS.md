...
- `src/jit/` is the JIT: `isa.rs` (one instruction decode), `abi.rs` (the frame contract shared with the entry trampolines), `validate.rs`, `patch.rs` (aarch64's code buffer and fixups), and `emit/{x86_64,aarch64}.rs`. The x86_64 backend is only an encoder: its decisions live in `src/verified/x64_lower.rs`, each macro's native sequence in `src/verified/x64_expand.rs`, and `src/verified/x64_check.rs` refuses any macro list that is not memory-safe (see `docs/jit-memory-safety.md`).
...
- `src/verified/` is the verified core: the instruction set, the load-time validator, the function layout, the guest stack's frame geometry, the region analysis' domain, transfer function, fixed point and whole-program live-in solver, and the x86_64 backend's lowering, expansion and memory-safety checker, which `jit::isa`, `jit::validate`, `function_analysis`, `program` and `region_analysis` call. The same directory is the library of the `lean/verified` crate that Charon and Aeneas translate to Lean, so the proofs under `lean/` (see `lean/README.md`) are about the code that runs. Keep it in the Aeneas-friendly subset described there, and after changing it run `make extract` in `lean/` and rebuild the proofs.
